...
public static class Config
{
    ...
    private static ShipCall GetShipCall(Container container)
    {
        ShipCall result = new();
        result.Route = GetRoute(container);
        container.GetKeyRing(result)!.SetField("ID_ROUTE", ++nextIdShipCall).SetField("ID_LINE", container.GetKeyRing(result.Route.Line)!["ID_LINE"]);
        ...
        {
            result.prevCall = GetShipCall(container);
        }
